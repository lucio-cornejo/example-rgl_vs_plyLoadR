...
plyLoadR(
  ...
  settings = list(isTransparent = list(TRUE, TRUE))
)
```
...
ViewsDates <- c("2022-01-05", "2022-01-11")
...
select_buttons <- function(n_of_mesh_left = length(FolderViews)) {
  if (n_of_mesh_left > 0 ) {
    return(paste0(
      '<p class="rangeLabel" data-child="',
      1 + length(FolderViews) - n_of_mesh_left, '">',
      ViewsDates[1 + length(FolderViews) - n_of_mesh_left],
      '</p>\n',
...
buttons_code <- select_buttons(length(FolderViews) - 1)
